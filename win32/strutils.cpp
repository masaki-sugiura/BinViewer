...
bool
IsValidPtr(LPCSTR ptr)
{
	return !::IsBadStringPtr(ptr, 1);
}
...
hextoi(LPCSTR val)
{
	...
	int ret = 0, n;
	for (;;) {
		if      (IsCharDigit(*val))  n = *val - '0';
		else if (IsCharXDigit(*val)) n = (*val & 0x5F) - 'A' + 10;
		else break;
		ret <<= 4;
		ret += n;
		val++;
